...
POST /api/save {lesson,score,missed,sentence,comments,peeks,furigana,seconds}
 -> memory/lesson-<id>.md + patch memory/global.md (append source, weak guess from missed/peeks)
"""
...
class H(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        with open(gpath, "a", encoding="utf-8") as f:
            f.write(f"\n- {lid}: score {data.get('score','')} missed {data.get('missed','')} peeks {data.get('peeks',[])}")
